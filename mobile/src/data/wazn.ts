// Wazn (صرف pattern) of a word — the morphological "measure" that its shape
// carries. Derived from the corpus morphology tags (verb_form I–XII, derivation
// = active/passive participle or verbal noun, aspect, voice), mapped to the
// canonical Arabic template. Templates are the abstract measure (ف‑ع‑ل); we show
// the root's three radicals beside them rather than fabricating a surface form,
// so weak/assimilated roots aren't misrepresented.

import type { Db } from "./db";

export interface Wazn {
  kind: "verb" | "active-participle" | "passive-participle" | "verbal-noun";
  form: string;          // roman numeral, e.g. "IV"
  wazn: string | null;   // the pattern, e.g. أَفْعَلَ (null for Form I masdar — no fixed measure)
  label: string;         // e.g. "Form IV verb"
  sense?: string;        // plain-language note on what the form tends to mean
  aspect?: string;       // verbs: past / present / command
  voice?: string;        // "passive" when applicable
  radicals?: string[];   // the root's letters, when triliteral
}
...
/** Compute the wazn descriptor for a word's morphology, or null when the word
 *  carries no derivational pattern (particles, plain nouns, proper nouns). */
...
/** The wazn of the tapped word (its STEM segment), or null. */
