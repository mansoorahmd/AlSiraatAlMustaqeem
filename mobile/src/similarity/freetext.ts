// Free-text Arabic query — on-device port of server/src/freetext.ts.
// Resolve each Arabic word to its most common Quranic root + POS, then route the
// root/POS sequences through the composite similarity engine. The engine is
// shared with the /similar path so its indexes are built only once.

import type { Db } from "../data/db";
import type { FreeTextResult } from "../types";
import { foldArabic, affixTrials, tokenizeArabic } from "../text/normalize";
import { SimilarityEngine, type DEFAULT_WEIGHTS } from "./compose";
...
export class FreeTextSearch {
  ...
  constructor(db: Db, private engine: SimilarityEngine) {
